...
function IsolatedExecutionEnvironment(config) {
    ...
    isolateGlobal.setSync('_logger', _deepReference(config.logger, ivm));
    ...
    function globalSetSync(name, value) {
        isolateGlobal.setSync(name, _deepReference(value, ivm));
    }
    ...
    this.ivm = ivm;
    this.createDeepReference = _deepReference;
}
